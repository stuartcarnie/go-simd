...
static inline void _memset(char *p, char v, size_t n) {
    for (int i = 0; i < n; i++) {
        p[i] = v;
    }
}

static inline void _memcpy(char *dst, const char *src, size_t n) {
    for (int i = 0; i < n; i++) {
        dst[i] = src[i];
    }
}
...
static bool validate_utf8_fast(state *state, const char *src, size_t len) {
  size_t i = 0;
  ...
  // last part
  if (i < len) {
    _memset(state->buffer, 0, 16);
    _memcpy(state->buffer, src + i, len - i);
    __m128i current_bytes = _mm_loadu_si128((const __m128i *)(state->buffer));
    previous = checkUTF8Bytes(current_bytes, &previous, &has_error);
  } else {
    has_error =
...
static bool validate_utf8_fast_avx_asciipath(state *state, const char *src, size_t len) {
  size_t i = 0;
  ...
  // last part
  if (i < len) {
    _memset(state->buffer, 0, 32);
    _memcpy(state->buffer, src + i, len - i);
    __m256i current_bytes = _mm256_loadu_si256((const __m256i *)(state->buffer));
...
static bool validate_utf8_fast_avx(state *state, const char *src, size_t len) {
  size_t i = 0;
  __m256i has_error = _mm256_setzero_si256();
  struct avx_processed_utf_bytes previous = {
      .rawbytes = _mm256_setzero_si256(),
